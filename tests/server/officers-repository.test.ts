...
class FakeD1Database implements D1Database {
  ...
  async batch(statements: D1PreparedStatement[]) {
    ...
    try {
      ...
      for (const statement of statements as FakePreparedStatement[]) {
        const result = this.mutate(statement.sql, statement.values);
  ...
  mutate(sql: string, values: unknown[]): MutationResult {
    ...
    if (query.startsWith("UPDATE memberships SET user_id = ?")) {
      const [userId, email, updatedAt, organizationId, id, expectedEmail, invitationId, invitationOrganizationId, tokenHash, invitationEmail, validAt] = values as [string, string, number, string, string, string, string, string, string, string, number];
      if (!this.eligibleInvitation(invitationId, invitationOrganizationId, tokenHash, invitationEmail, validAt)) {
        return { success: true, meta: { changes: 0 } };
      }
      const existing = this.memberships.find((row) => row.organization_id === organizationId && row.id === id && row.email.trim().toLowerCase() === expectedEmail);
    ...
    if (query.startsWith("INSERT INTO memberships") && query.includes("FROM invitations")) {
      ...
      if (query.includes("NOT EXISTS") && this.memberships.some((row) => row.organization_id === String(values[11]) && row.email.trim().toLowerCase() === String(values[12]))) {
        return { success: true, meta: { changes: 0 } };
      }
    ...
    if (query.startsWith("UPDATE invitations SET accepted_at = ?")) {
      const [acceptedAt, id, organizationId, tokenHash, email, validAt, membershipId, membershipOrganizationId, userId, membershipEmail] = values as [number, string, string, string, string, number, string, string, string, string];
      if (query.includes("changes() = 1") && this.lastChanges !== 1) return { success: true, meta: { changes: 0 } };
      ...
      const ready = this.memberships.some((row) => row.id === membershipId && row.organization_id === membershipOrganizationId && row.user_id === userId && row.email.trim().toLowerCase() === membershipEmail && row.active === 1);
      if (!ready) return { success: true, meta: { changes: 0 } };
      eligible.accepted_at = acceptedAt;
      return { success: true, meta: { changes: 1 } };
    }
    if (query.startsWith("UPDATE invitations SET invalidated_at = ?") && query.includes("id <> ?")) {
      const [invalidatedAt, organizationId, email, excludedId, auditId, auditOrganizationId, entityId] = values as [number, string, string, string, string, string, string];
      const acceptanceAudit = this.auditEvents.find((row) => row.id === auditId && row.organization_id === auditOrganizationId && row.action === "invitation_accepted" && row.entity_id === entityId);
      if (!acceptanceAudit) return { success: true, meta: { changes: 0 } };
      let changes = 0;
    ...
    if (query.startsWith("UPDATE invitations SET invalidated_at = ?")) {
      ...
      if (query.includes("NOT EXISTS") && this.memberships.some((row) => row.organization_id === String(values[4]) && row.email.trim().toLowerCase() === String(values[5]) && row.active === 1)) {
        return { success: true, meta: { changes: 0 } };
      }
      let changes = 0;
      for (const row of this.invitations) {
    ...
    if (query.startsWith("INSERT INTO invitations")) {
      ...
      if (query.includes("NOT EXISTS") && this.memberships.some((row) => row.organization_id === String(values[7]) && row.email.trim().toLowerCase() === String(values[8]) && row.active === 1)) {
        return { success: true, meta: { changes: 0 } };
      }
    ...
    if (query.startsWith("INSERT INTO audit_events")) {
      if (query.includes("FROM memberships AS target")) {
      ...
      if (query.includes("FROM invitations")) {
        if (action === "invitation_created") {
          const [invitationId, invitationOrganizationId, tokenHash] = values.slice(7) as [string, string, string];
          const createdInvitation = this.invitations.find((row) => row.id === invitationId && row.organization_id === invitationOrganizationId && row.token_hash === tokenHash && row.accepted_at === null && row.invalidated_at === null);
          if (this.lastChanges !== 1 || !createdInvitation) return { success: true, meta: { changes: 0 } };
        } else {
          const [invitationId, invitationOrganizationId, tokenHash, invitationEmail, acceptedAt, membershipId, membershipOrganizationId, userId, membershipEmail] = values.slice(7) as [string, string, string, string, number, string, string, string, string];
          const acceptedInvitation = this.invitations.find((row) => row.id === invitationId && row.organization_id === invitationOrganizationId && row.token_hash === tokenHash && row.email.trim().toLowerCase() === invitationEmail && row.accepted_at === acceptedAt);
          const ready = this.memberships.some((row) => row.id === membershipId && row.organization_id === membershipOrganizationId && row.user_id === userId && row.email.trim().toLowerCase() === membershipEmail && row.active === 1);
          if (this.lastChanges !== 1 || !acceptedInvitation || !ready) return { success: true, meta: { changes: 0 } };
        }
      }
      this.auditEvents.push({
        id,
...
describe("officer repository lifecycle", () => {
  ...
  it("generates an unpadded base64url token containing exactly 32 random bytes", async () => {
    const database = new FakeD1Database();

    const result = await createOfficerInvitation(environment(database), actor, "token@stuco.example", "https://stuco.example", new Date("2026-08-26T12:00:00.000Z"));
  ...
  it("does not mutate storage when the invitation base URL is invalid", async () => {
    const prior = invitation({ id: "prior", email: "invalid-url@stuco.example" });
    const database = new FakeD1Database({ invitations: [prior] });
    const initialState = structuredClone({ invitations: database.invitations, auditEvents: database.auditEvents });

    await expect(createOfficerInvitation(environment(database), actor, "invalid-url@stuco.example", "not a valid absolute URL", new Date("2026-08-26T12:00:00.000Z")))
      .rejects.toThrow();

    expect({ invitations: database.invitations, auditEvents: database.auditEvents }).toEqual(initialState);
  });

  it("rechecks active membership in the creation batch when activation races the preflight read", async () => {
    const prior = invitation({ id: "prior", email: "racing@stuco.example" });
    const database = new FakeD1Database({ invitations: [prior] });
    const initialInvitations = structuredClone(database.invitations);
    database.beforeBatch = () => {
      database.memberships.push(membership({ id: "racing-active", email: "racing@stuco.example" }));
    };

    await expect(createOfficerInvitation(environment(database), actor, "racing@stuco.example", "https://stuco.example", new Date("2026-08-26T12:00:00.000Z")))
      .rejects.toMatchObject({ code: "ACTIVE_MEMBER_EXISTS" });

    expect(database.invitations).toEqual(initialInvitations);
  ...
  it("does not consume or audit an invitation when its preselected membership disappears before the batch", async () => {
    const existing = membership({ id: "disappearing", email: "invitee@stuco.example", active: 0 });
    const sibling = invitation({ id: "sibling", token_hash: createHash("sha256").update("sibling").digest("hex") });
    const database = new FakeD1Database({ memberships: [existing], invitations: [invitation({ token_hash: rawInvitationTokenHash }), sibling] });
    database.beforeBatch = () => {
      database.memberships = database.memberships.filter((row) => row.id !== "disappearing");
    };

    await expect(acceptOfficerInvitation(environment(database), acceptingActor, "org-a", rawInvitationToken, new Date("2026-08-26T12:00:00.000Z")))
      .rejects.toMatchObject({ code: "INVITATION_INVALID" });

    expect(database.memberships).toEqual([]);
    expect(database.invitations.find((row) => row.id === "invitation-a")?.accepted_at).toBeNull();
    expect(database.invitations.find((row) => row.id === "sibling")?.invalidated_at).toBeNull();
    expect(database.auditEvents).toEqual([]);
  });
  ...
  it("rejects a cross-organization target without reading or mutating that membership", async () => {
    ...
    await expect(deactivateOfficer(environment(database), actor, "foreign-target", new Date("2026-08-26T12:00:00.000Z")))
      .rejects.toMatchObject({ code: "OFFICER_NOT_FOUND" });

    expect(database.memberships).toEqual(initialMemberships);
    expect(database.auditEvents).toEqual([]);
  });

  it("returns a typed not-found error for an inactive scoped target", async () => {
    const database = new FakeD1Database({
      memberships: [
        membership({ id: "inactive-target", active: 0 }),
        membership({ id: "remaining", email: "remaining@stuco.example" }),
      ],
    });
    const initialMemberships = structuredClone(database.memberships);

    await expect(deactivateOfficer(environment(database), actor, "inactive-target", new Date("2026-08-26T12:00:00.000Z")))
      .rejects.toMatchObject({ code: "OFFICER_NOT_FOUND" });
